...
public class RtpPlayer extends Thread implements ReceiveStreamListener,
	...
	public void run() {
		...
		try {
			...
			LOGGER.error("error determinig local IP address", e);
			return;
		}
		...
		try {
			...
			LOGGER.error("error initialzing RTP manager", e);
			return;
		} catch (IOException e) {
			LOGGER.error("error initialzing RTP manager", e);
			return;
		...
		try {
			...
			LOGGER.error("error adding target address", e);
			return;
		} catch (IOException e) {
			LOGGER.error("error adding target address", e);
			return;
