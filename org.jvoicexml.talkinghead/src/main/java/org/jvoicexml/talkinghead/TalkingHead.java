...
public class TalkingHead
        implements Observer, BMLExecutor.Listener, MaryTTSClient.Listener,
        Animator.Listener {
    /**
     ...
     */
    public TalkingHead() throws IOException, JDOMException,
        ...
        int inputBufferSize = appConfig
                .getInputBuffer(DEFAULT_NETWORK_BUFFER_SIZE);
        ...
        int outputBufferSize = appConfig
                .getOutputBuffer(DEFAULT_NETWORK_BUFFER_SIZE);
        talkingHeadOutputConnection = new UDPConnection(outputBufferSize);
        talkingHeadOutputConnection.connect(
                appConfig.getOutputHost("127.0.0.1"),
        ...
        try {
            ...
            talkingHeadTextToSpeechProcessor = new MaryTTSClient(ttsIP,
                    ttsPort, ttsLocale, ttsVoice, this);
        } catch (Exception exc) {
            exc.printStackTrace();
    ...
    public final void update(final BMLEvent event) {
        ...
        case BMLEvent.TYPE_START:
            ...
            case ITag.TYPE_GESTURE:
                ...
                // Get Animation
                Animation anim = talkingHeadAvatarModel.getAnimation(eventTag
                        .getLexeme());
            ...
            case ITag.TYPE_SPEECH:
                ...
                try {
                    if (talkingHeadTextToSpeechProcessor != null) {
                        talkingHeadTextToSpeechProcessor.process(event
                                .getCommand().getID(), speech.getText());
                    } else {
            ...
            case ITag.TYPE_POINTING:
                // Get Animation
                Animation showAnimation = talkingHeadAvatarModel
                        .getAnimation("show");
        ...
        case BMLEvent.TYPE_STOP:
            talkingHeadOutputConnection.send("<event id=\""
                    + event.getCommand().getID() + ":end\" />");
            talkingHeadAnimator.stopAnimation(event.getCommand().getID());
            break;
        case BMLEvent.TYPE_START_BML:
            if (talkingHeadOutputConnection != null && event != null
                    && event.getBML() != null) {
                talkingHeadOutputConnection.send("<event id=\""
                        + event.getBML().getID() + ":start\" />");
            }
            break;
        case BMLEvent.TYPE_STOP_BML:
            if (event != null && event.getBML() != null
                    && talkingHeadOutputConnection != null) {
                talkingHeadOutputConnection.send("<event id=\""
                        + event.getBML().getID() + ":end\" />");
            }
            break;
        default:
            break;
     ...
     */
    @Override
    public final void update(final int type, final String processID) {
        if (type == MaryTTSClient.Listener.EVENT_TYPE_START) {
            ...
            talkingHeadViewer.setStatusDisplay("Status: [" + processID
                    + "] speak");
    ...
    public final void update(final AnimationEvent event) {
        ...
        case AnimationEvent.START_EVENT:
            talkingHeadViewer.setStatusDisplay("Status: [" + event.getID()
                    + "] ");
            break;
        case AnimationEvent.STOP_EVENT:
            if (talkingHeadBMLExecutor != null) {
                talkingHeadBMLExecutor.triggerEvent(event.getID() + ":end");
            }
        ...
        case AnimationEvent.NEXT_FRAME_EVENT:
            ...
            // Get Image
            Image frame = talkingHeadAvatarModel.getImage(nextFrameEvent
                    .getFrame());
