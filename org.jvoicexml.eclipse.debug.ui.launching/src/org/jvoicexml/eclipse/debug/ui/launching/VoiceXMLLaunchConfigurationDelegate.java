...
public class VoiceXMLLaunchConfigurationDelegate
    ...
    public void launch(ILaunchConfiguration configuration, String mode,
        ...
        String url = configuration.getAttribute(
                IVoiceXMLBrowserConstants.LAUNCH_URL, ""); //$NON-NLS-1$
        if (url == null || url.trim().length() == 0) {
            return;
        }
        String browserID = configuration.getAttribute(
            ...
            for (int i = 0; browser == null && i < extensions.length; i++) {
                ...
                for (int j = 0; browser == null && j < elements.length; j++) {
                    ...
                    if (id != null && id.equals(browserID)) {
                        ...
                        String launchInputView = elements[j]
                                .getAttributeAsIs("launchInputView"); //$NON-NLS-1$
                        if (launchInputView != null
                                && launchInputView.trim().toLowerCase()
                                    ...
                                    public void run() {
                                        try {
                                            PlatformUI
                                                    ...
                                                    .getActivePage()
                                                    .showView(
                                                            "org.jvoicexml.eclipse.debug.ui.inputView", null, IWorkbenchPage.VIEW_ACTIVATE); //$NON-NLS-1$
        ...
        VoiceXMLBrowserProcess proc = new VoiceXMLBrowserProcess(launch,
                browser);
