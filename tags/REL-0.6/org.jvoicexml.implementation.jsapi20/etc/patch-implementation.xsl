...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:copy>
   ...
   <list>
     <xsl:for-each select="beans:list/beans:bean">
        <xsl:if test="substring(@class, 1, 36) != 'org.jvoicexml.implementation.jsapi10'">
            <xsl:apply-templates select="@*|*|text()|comment()" />
        </xsl:if>
     </xsl:for-each>
     <bean
      class="org.jvoicexml.implementation.jsapi20.jvxml.SynthesizedOutputFactory">
 ...
 <xsl:template match="beans:property[@name='spokeninput']">
  <xsl:copy>
   ...
   <list>
    ...
    <bean
     class="org.jvoicexml.implementation.jsapi20.jvxml.SpokenInputFactory">
     <property name="instances" value="1" />
    </bean>
   </list>
  </xsl:copy>
 ...
 <xsl:template match="beans:bean[substring(@class, 1, 36) = 'org.jvoicexml.implementation.jsapi10']">
 </xsl:template>
