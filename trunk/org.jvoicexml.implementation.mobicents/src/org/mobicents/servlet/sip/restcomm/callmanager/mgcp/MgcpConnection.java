...
public final class MgcpConnection extends FiniteStateMachine implements JainMgcpListener {
    ...
    public void connect(final ConnectionMode connectionMode) throws MgcpConnectionException {
        ...
        try {
            ...
            LOGGER.debug("send crcx:"+crcx + " with callId:"+callId);
    ...
    public void connect(final ConnectionMode connectionMode,SipServletRequest initialInvite) throws MgcpConnectionException {
        assertState(CREATED);
        try {
            final CallIdentifier callId = new CallIdentifier(Integer.toString(session.getId()));
            final CreateConnection crcx = new CreateConnection(this, callId, endpoint.getId(), connectionMode);
            ...
            server.sendCommand(crcx, this);
            LOGGER.debug("send crcx:"+crcx + " with sipCallId:"+initialInvite.getCallId());
        } catch (final MgcpServerException exception) {
            setState(FAILED);
            fireFailed();
            throw new MgcpConnectionException(exception);
...
//        modify(remoteDescriptor, ConnectionMode.SendRecv);
    ...
    {
        LOGGER.debug("...with ConnectionMode:"+connectionMode);
    ...
    @Override
    public void processMgcpCommandEvent(final JainMgcpCommandEvent ignored) {
        // Nothing to do.
    }
    ...
    public void processMgcpResponseEvent(final JainMgcpResponseEvent response) {
        ...
        if (returnCode.getValue() == ReturnCode.TRANSACTION_BEING_EXECUTED) {
            ...
            setState(FAILED);
            fireFailed();
            final String error = new StringBuilder().append(returnCode.getValue()).append(" ").append(returnCode.getComment()).toString();
            LOGGER.error(error);
        }
    }
