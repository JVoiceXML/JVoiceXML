...
some resources are not closed propertly and may cause conflicts with the next
start of JVoiceXML.
...
Microsofts speech engine. This implementation platform requires that you have
...
can be obtained from conversay's web page
...
from conversay's web page conversations~\cite{conversay:jsr113}. The
...
To make use of this implementation platform, JVoicXML must be called with a SIP
 ...
 * JVoiceXML group, $LastChangedDate $,
...
ls\lstinline{org.jvoicexml} project to this new folder:
\begin{enumerate}
  \item nightly.xml
...
copying the contents of the personal-props directoy of your development copy
...
classes written for JVoiceXML. The unit tests are started from the core project
\lstinline{org.jvoicexml}. It is the entry point of the unit tests of the
various subproject. Therefore, change to the core directoy of the release
copy and call
\begin{lstlisting}
cd org.jvoicexml
ant -f nightly.xml unittests
\end{lstlisting}
This will take some time. The results of the unit tests of all subprojects can
be viewed using your favourite web broser by opening
...
Uploading requires the following propertis in your personal copy of the ANT
properties:
\begin{lstlisting}
nightly.sf.user=
nightly.sf.password=
...
to make a new relase. & Dirk Schnelle-Walka & 04/14/2010 \\
\hline
\end{tabular}
