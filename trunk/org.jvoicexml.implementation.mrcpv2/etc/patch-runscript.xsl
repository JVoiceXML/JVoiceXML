...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="path[@id='run.classpath']">
    <xsl:copy>
      ...
      <fileset id="jvxml.mrcpv2.lib">
        <xsl:attribute name="dir">
            <xsl:value-of select="$distpath" />
        </xsl:attribute>
        <include name="jvxml-mrcpv2.jar" />
      </fileset>
      <fileset id="cairo.lib">
        <xsl:attribute name="dir">
          <xsl:value-of select="concat($thirdparty, '/cairo/lib')" />
        </xsl:attribute>
        <include name="*.jar" />
      </fileset>
      ...
      <fileset id="mrcp4j.lib">
        <xsl:attribute name="dir">
          <xsl:value-of select="concat($thirdparty, '/mrcp4j/lib')" />
        </xsl:attribute>
        <include name="*.jar" />
      </fileset>
    </xsl:copy>
