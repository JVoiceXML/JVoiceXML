...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="path[@id='run.classpath']">
    <xsl:copy>
      ...
      <fileset id="jvxml.jsapi10.lib">
        <xsl:attribute name="dir">
            <xsl:value-of select="$distpath" />
        </xsl:attribute>
        <include name="jvxml-jsapi1.0.jar" />
        <include name="jvxml-jsapi1.0-impl.jar" />
      </fileset>
      <fileset id="jsapi1.lib">
        <xsl:attribute name="dir">
       <xsl:value-of select="concat($thirdparty, '/jsapi1.0/lib')" />
     </xsl:attribute>
        <include name="*.jar" />
      </fileset>
