...
\title{JVoiceXML User Guide}
\version{\jvxmlversion}
...
voice browser and how ti write custom extensions.
...
differently or should be added. Please, use the issues at GitHub for any
such request at \url{https://github.com/JVoiceXML/JVoiceXML/issues}.
This is also a good source to check for other open issues or limitations
of JVoiceXML. 
...
editor or preferably a JAVA IDE, see section~\ref{sec:ide}, and Gradle, see 
section~\ref{sec:gradle}, to build and run the binaries for the 
...
The demos are being built by Gradle. It is recommended that
...
The demos of this user guide do not rely on Gradle, so you do not need to install
Gradle if you play with the examples of this user guide.
...
You can download the compiled voice browser as \texttt{jvoicexml-VERSION.zip} from 
\url{https://github.com/JVoiceXML/JVoiceXML/releases}.
\texttt{VERSION} has to be replaced by the used version number, e.g. 0.7.9.
Unpack the zipped distribution file to install the voicebrowser into a directory of your
choice. In the rest of this document this directory will be referred as \lstinline{JVOICEXML_HOME}.

JVoiceXML is shipped with different implementation platforms which are all
...
download Cairo from \url{https://github.com/JVoiceXML/cairo}. Cairo makes use of JMF
...
that JMF is outdated and does not work well with 64bit Java under Windows.
...
For our
tests we use Jitsi Desktop (\url{https://desktop.jitsi.org/}).
We are using a registrarless configuration. In case you are using any other 
phone, please deactivate options like
\emph{Register with domain and receive calls}.
...
If you want to disable a configuration, the recommended way is to create a
subfolder in \texttt{JVOICEXML\_HOME/config} and move the unused configuration
files there.
...
The first mode is pretty much what you may know from other voice browsers. Here,
...
The shell script \texttt{jvxml-startup.sh} located in the \texttt{bin} folder
of your JVoiceXML installation can be used to start the browser.

This should be called from the directory where you installed JVoiceXML.
Simply call

\begin{lstlisting}
cd JVOICEXML_HOME
sh ./bin/jvxml-startup.sh
\end{lstlisting}
...
The \texttt{bin} folder also contains the files to stop the browser. 
...
The shell script \texttt{jvxml-stutdown.sh} located in the \texttt{bin} folder
...
easier if I know the problems with it. The preferred way is over issues in 
GitHub~\url{https://github.com/JVoiceXML/JVoiceXML/issues}.

\section{Running the Demos}

The browser comes with some demo programs. You'll find them in the
directory \texttt{JVOICEXML\_HOME/demo}. Use the IDE of your choice or a text
editor and explore there contents. Some features of the browser can 
become more clear with them.

Most of the demos are designed to work with the JSAPI 2.0 implementation
platform. Make sure that you have this configuration enabled before running the
demos.

The demo programs feature an Gradle script which can be used for starting.
See also section~\ref{sec:required-software} before running the demos.

In most cases it should be sufficient to change to each demo directory
and call after JVoiceXML has been started
...
\emph{HelloWorldServletDemo}.

Before you can run this demo, create a war file from the project
\texttt{org.jvoicexml.demo.helloworldservletdemo.servlet} via

\begin{lstlisting}
gradle war
\end{lstlisting}

This war archive must be deployed to you servlet container
...
\lstinputlisting[language=XML]{../org.jvoicexml.implementation.jsapi20/src/main/config/jsapi20-implementation.xml}

\section{JSAPI 2.0 with Microsoft SAPI}
\lstinputlisting[language=XML]{../org.jvoicexml.implementation.jsapi20/src/main/config/jsapi20-sapi-implementation.xml}

\section{JTAPI}
\lstinputlisting[language=XML]{../org.jvoicexml.implementation.jtapi/src/main/config/jtapi-implementation.xml}

\section{MARY}
\lstinputlisting[language=XML]{../org.jvoicexml.implementation.mary/src/main/config/mary-implementation.xml}

\section{MMI with HTTP ETL}
\lstinputlisting[language=XML]{../org.jvoicexml.callmanager.mmi.http/src/main/config/mmi-callmanager-http.xml}

\section{MMI with Socket ETL}
\lstinputlisting[language=XML]{../org.jvoicexml.callmanager.mmi.socket/src/main/config/mmi-callmanager-socket.xml}

\section{MRCPv2}
\lstinputlisting[language=XML]{../org.jvoicexml.callmanager.sip/src/main/config/sip-callmanager.xml}

\lstinputlisting[language=XML]{../org.jvoicexml.implementation.mrcpv2/src/main/config/mrcpv2-implementation.xml}

\section{Text}
\lstinputlisting[language=XML]{../org.jvoicexml.implementation.text/src/main/config/text-implementation.xml}
