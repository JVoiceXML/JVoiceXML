...
JVoiceXML is being developed using eclipse 4.4, but you can use the IDE of your
...
We are using the \emph{Eclipse IDE for Java EE Developers} version 4.4 (Luna))
...
since this may cause some interference with the ANT build
...
Since almost all IDEs feature an ANT integration, it will be possible to use
your favorite IDE. However, it might take you some manual effort to add all the needed libraries.
...
directly may result in strange errors and a huge amount of adaptations to the
classpath.
...
folder. These subfolder follow the following convention:
...
\texttt{build.xml}. All others build files from the implementation platforms
