...
\title{JVoiceXML 0.7.4 User Guide}
\version{0.7.4.1}
...
you need to install the speech SDK 5.1 for Windows in advance.
...
Follow the instructions to install Cairo and JMF. Then, start Cairo.
For our tests we use xlite
(\url{http://www.counterpath.com/x-lite.html}). Create a new account and make
sure that the settings are as shown in figure~\ref{fig:sip-properties}.
\begin{figure}
\includegraphics[width=.5\linewidth]{SIPProperties1}
\includegraphics[width=.5\linewidth]{SIPProperties2}
...
the Cairo Server. The entry for the \lstinline{applications} of the
\lstinline{callmanager} bean should have a key matching the user name of your
SIP phone and a value of the URL to your VoiceXML application.

Note that it may also be necessary to change the RMI port if you are running
Cairo on the same machine.
