...
connection will not work, if cairo is not present when JVoiceXML starts. For our
tests we use xlite (\url{http://www.counterpath.com/x-lite.html}).
...
\lstinline{callmanager} specify which SIP extension number 
is associated with which URL to your VoiceXML application.
...
The argument on the \texttt{createSession} is a
...
The argument for \texttt{session.call(URI)} must point to to URI of the
