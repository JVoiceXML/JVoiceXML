...
public class VoiceXMLLaunchConfigurationDelegate implements
	...
	public void launch(ILaunchConfiguration configuration, String mode,
		...
		IExtensionPoint iep = Platform.getExtensionRegistry().getExtensionPoint("org.jvoicexml.eclipse.debug.ui.launching.voiceXMLBrowser"); //$NON-NLS-1$
		if (iep != null) {
						...
						if (launchInputView != null && launchInputView.trim().toLowerCase().equals("true")) { //$NON-NLS-1$
							try {
								Display.getDefault().syncExec(new Runnable() {
									public void run() {
										try {
											PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().showView("org.jvoicexml.eclipse.debug.ui.inputView", null, IWorkbenchPage.VIEW_ACTIVATE); //$NON-NLS-1$
